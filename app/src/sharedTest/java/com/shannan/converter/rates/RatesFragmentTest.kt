...
import com.shannan.converter.util.saveTaskBlocking
...
class RatesFragmentTest {
    ...
    fun initRepository() {
        ...
        ServiceLocator.ratesRepository = repository
    }
    ...
    fun displayTask_whenRepositoryHasData() {
        ...
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1"))
    ...
    fun displayCompletedTask() {
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1", true))
    ...
    fun deleteOneOfTwoTasks() {
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1"))
        repository.saveTaskBlocking(RatesResponse("TITLE2", "DESCRIPTION2"))
    ...
    fun showAllTasks() {
        ...
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1"))
        repository.saveTaskBlocking(RatesResponse("TITLE2", "DESCRIPTION2", true))

        launchActivity()

        // Verify that both of our rates are shown
        onView(withId(R.id.menu_filter)).perform(click())
        onView(withText(R.string.nav_all)).perform(click())
        onView(withText("TITLE1")).check(matches(isDisplayed()))
        onView(withText("TITLE2")).check(matches(isDisplayed()))
    ...
    @Test
    fun showActiveTasks() {
        // Add 2 active rates and one completed task
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1"))
        repository.saveTaskBlocking(RatesResponse("TITLE2", "DESCRIPTION2"))
        repository.saveTaskBlocking(RatesResponse("TITLE3", "DESCRIPTION3", true))

        launchActivity()

        // Verify that the active rates (but not the completed task) are shown
        onView(withId(R.id.menu_filter)).perform(click())
        onView(withText(R.string.nav_active)).perform(click())
        onView(withText("TITLE1")).check(matches(isDisplayed()))
        onView(withText("TITLE2")).check(matches(isDisplayed()))
    ...
    fun showCompletedTasks() {
        // Add one active task and 2 completed rates
        repository.saveTaskBlocking(RatesResponse("TITLE1", "DESCRIPTION1"))
        repository.saveTaskBlocking(RatesResponse("TITLE2", "DESCRIPTION2", true))
        repository.saveTaskBlocking(RatesResponse("TITLE3", "DESCRIPTION3", true))
        ...
        // Verify that the completed rates (but not the active task) are shown
        onView(withId(R.id.menu_filter)).perform(click())
        onView(withText(R.string.nav_completed)).perform(click())
        onView(withText("TITLE1")).check(doesNotExist())
        onView(withText("TITLE2")).check(matches(isDisplayed()))
    ...
    fun noTasks_AllTasksFilter_AddTaskViewVisible() {
        ...
        // Verify the "You have no rates!" text is shown
        onView(withText("You have no rates!")).check(matches(isDisplayed()))
    }
    ...
    fun noTasks_CompletedTasksFilter_AddTaskViewNotVisible() {
        ...
        // Verify the "You have no completed rates!" text is shown
        onView(withText("You have no completed rates!")).check(matches((isDisplayed())))
    }
    ...
    fun noTasks_ActiveTasksFilter_AddTaskViewNotVisible() {
        ...
        // Verify the "You have no active rates!" text is shown
        onView(withText("You have no active rates!")).check(matches((isDisplayed())))
    }

    @Test
    fun clickAddTaskButton_navigateToAddEditFragment() {
        // GIVEN - On the home screen
        val scenario = launchFragmentInContainer<RatesFragment>(Bundle(), R.style.AppTheme)
    ...
    private fun launchActivity(): ActivityScenario<RatesActivity>? {
        val activityScenario = launch(RatesActivity::class.java)
        activityScenario.onActivity { activity ->
            // Disable animations in RecyclerView
            (activity.findViewById(R.id.rates_list) as RecyclerView).itemAnimator = null
        }
